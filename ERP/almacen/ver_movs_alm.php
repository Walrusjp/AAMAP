...
$id_alm = isset($_GET['id_alm']) ? intval($_GET['id_alm']) : 0;

// 3. Obtener información del artículo si tenemos id_alm
$articulo_info = ['codigo' => 'Todos los artículos', 'descripcion' => ''];
if ($id_alm > 0) {
    $sql_articulo = "SELECT codigo, descripcion FROM inventario_almacen WHERE id_alm = $id_alm";
    $result_articulo = $conn->query($sql_articulo);
    
    if ($result_articulo && $result_articulo->num_rows > 0) {
        $articulo_info = $result_articulo->fetch_assoc();
    }
}

// 4. Consulta principal de movimientos
...
<head>
    ...
    <title><?php echo htmlspecialchars($articulo_info['codigo']); ?> </title>
    <style>
        /* Estilos generales */
        body {
...
<body>
    <div class="container">
        <div class="header">
            <h3>Historial de Movimientos - Artículo: <?php 
                echo htmlspecialchars($articulo_info['codigo']);
                if (!empty($articulo_info['descripcion'])) {
                    echo " || " . htmlspecialchars($articulo_info['descripcion']);
                }
            ?></h3>
